...
| hover-stop-propagation | bool   | false  | 否   | 指定是否阻止本节点的祖先节点出现点击态                      |
...
+ 使用`@import`语句可以导入外联样式表，`@import`后跟需要导入的`外联样式表的相对路径`，用`;`表示语句结束
...
<template>
	<view>
		<button type="default" @click="setData">给test1传值</button>
	</view>
</template>
...
<script>
	export default {
		...
		methods:{
			setData(){
				uni.$emit('unpdataNum','我是来值test的值')
			}
		}
	}
</script>
